...
const createSchedule = async (req: Request, res: Response) => {
  try {
    ...
    // Verificar que no exista ya un horario para ese campo, día y hora
    const existingSchedule = await Schedule.findOne({ field, day, time });
    if (existingSchedule) {
      return res.status(400).json({
    ...
    const schedule = new Schedule({ field, day, time });
    await schedule.save();

    // Populate para devolver la info completa del campo
    ...
    res.status(500).json({ message: "Error creating schedule", error: true });
  }
};
...
const createBulkSchedules = async (req: Request, res: Response) => {
  try {
    ...
    // Generar todas las combinaciones de día y hora
...
const updateSchedule = async (req: Request, res: Response) => {
  try {
    ...
    // Validar formato de hora si se está actualizando
